...
void printVetoWindows()
{
	...
	for (int layer = 0; layer < 4; layer++)
	{
		f_win[layer] = new TF1(Form("f_win_%i", layer), Form("(x < %g)*%g + (x >= %g && x < %g)*(%g + %g / ( x + %g)) + (x >= %g)*%g", ptcut[layer][0], veto_limit[layer][0], ptcut[layer][0], ptcut[layer][1], par[layer][0], par[layer][1], par[layer][2], ptcut[layer][1], veto_limit[layer][1]), 0.0, 9.0);
		cout << Form("(x < %g)*%g + (x >= %g && x < %g)*(%g + %g / ( x + %g)) + (x >= %g)*%g", ptcut[layer][0], veto_limit[layer][0], ptcut[layer][0], ptcut[layer][1], par[layer][0], par[layer][1], par[layer][2], ptcut[layer][1], veto_limit[layer][1]) << endl;
	}
	...
	f_win[0]->GetYaxis()->SetRangeUser(0.0, 0.1);
	f_win[0]->Draw();
	TLatex latex;
	latex.SetNDC();
	...
	f_win[1]->GetYaxis()->SetRangeUser(0.0, 0.1);
	f_win[1]->Draw();
	...
	f_win[2]->GetYaxis()->SetRangeUser(0.0, 0.1);
	f_win[2]->Draw();
	...
	f_win[3]->GetYaxis()->SetRangeUser(0.0, 0.1);
	f_win[3]->Draw();
